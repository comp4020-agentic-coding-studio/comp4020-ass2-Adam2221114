...
1. **Per-lab images + quiz.** Real tutorial text for all 10 labs now exists (migrated from the former `sessions` collection — see above). Still needed: images, and 3 multiple-choice questions per lab where a learner gets immediate correct/incorrect feedback right after submitting an answer.
2. **Starter placeholder photos.** `pnpm check:evidence` currently fails on two pre-existing findings unrelated to the timeline work above: `src/content/people/idris-fenn.avif` and `marisol-quaye.avif` are still the starter template's images and need replacing (or removing per that check's own guidance).
